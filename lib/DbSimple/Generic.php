...
class DbSimple_Generic_Database extends DbSimple_Generic_LastError
{
     ...
     */
    function ____________PROTECTED()
    {
    } // for phpEclipse outline
    ...
    function _logQueryStat($queryTime, $fetchTime, $firstFetchTime, $rows)
    {
        ...
        if ($firstFetchTime + $tailFetchTime) {
            $log = sprintf("  -- %d ms = %d+%d" . ($tailFetchTime ? "+%d"
                : ""), $dt, $dt - $firstFetchTime - $tailFetchTime, $firstFetchTime, $tailFetchTime);
        } else {
            $log = sprintf("  -- %d ms", $dt);
        }
        $log .= "; returned ";
